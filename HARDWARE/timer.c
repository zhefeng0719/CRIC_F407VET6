...
void TIM12_SERVO_Init(u16 arr,u16 psc)//此定时器做定时中断
{
	 // 使能定时器12时钟
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM12, ENABLE);
    // 配置定时器参数
    TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
    TIM_TimeBaseStructure.TIM_Period = 219;           // 设置自动重装载寄存器的值（计数最大值）
    TIM_TimeBaseStructure.TIM_Prescaler = 83;         // 设置预分频器（使得计数器频率为1kHz）
    TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;
